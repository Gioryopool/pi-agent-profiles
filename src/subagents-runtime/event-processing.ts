...
export function processSubagentEvent(previous: LiveActivity, event: unknown): LiveActivity {
  ...
  const activity = tool ?? (type === "message_end" ? "assistant response" : type === "message_start" ? "assistant thinking" : type);
  ...
  if (complete && activity) {
    trail = [...trail, { label: current?.label ?? activity }].slice(-TRAIL_LIMIT);
    current = undefined;
  } else if (activity && (start || /_update$/.test(type ?? ""))) {
    if (start && current && current.label !== activity) trail = [...trail, current].slice(-TRAIL_LIMIT);
