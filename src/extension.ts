...
import { DEFAULT_SHORTCUT, MODEL_ROUTE_EVENT, STATUS_KEY } from "./constants.js";
import { ProfileManager } from "./profile-manager.js";
import { internalAgentCatalog, readCompatibleSubagentsConfig } from "./subagents-runtime/discovery.js";
...
function readGlobalConfigSync(path: string): Config {
  try {
    return validateConfig(JSON.parse(readFileSync(path, "utf8"))).config ?? emptyConfig();
  } catch {
    return emptyConfig();
  }
}
...
const supportedShortcut = (key: string) =>
  /^(?:(?:ctrl|alt|shift|meta)\+)+(?:[a-z0-9]|tab|f(?:[1-9]|1[0-2]))$/i.test(key);
...
const terminalInputMatchesShortcut = (text: string, shortcut: string) => {
  ...
  return Boolean(match && text.length === 1 && text.charCodeAt(0) === match[1].toLowerCase().charCodeAt(0) - 96);
};

export type ExtensionDependencies = { runtimeManagerFactory?: (config: CompatibleSubagentsConfig) => ForegroundTaskManager; runtimeHistoryFactory?: (config: CompatibleSubagentsConfig) => RuntimeHistory };
...
export default function extension(pi: PiLike, dependencies: ExtensionDependencies = {}) {
  ...
  const widgetKey = (sessionId: string) => `pi-agent-profiles:background:${sessionId}`;
  const runtimeFs = {
    exists(path: string) { try { return existsSync(path); } catch { return false; } },
    readFile(path: string) { try { return readFileSync(path, "utf8"); } catch { return undefined; } },
    readDir(path: string) { try { return readdirSync(path); } catch { return []; } },
  };
  const globalPath = join(getAgentDir(), "pi-agent-profiles", "config.json");
  const initialRuntimeConfig = readCompatibleSubagentsConfig({ fs: runtimeFs, agentDir: getAgentDir(), cwd: process.cwd(), projectTrusted: false });
  const backgroundHandoffShortcut = initialRuntimeConfig.backgroundHandoffShortcut ?? "ctrl+h";
  // Pi shortcut registration is construction-time: history remains a single safe global Ctrl+, chord.
  const historyPanelShortcut = initialRuntimeConfig.historyPanelShortcut ?? "ctrl+,";
  const runtimeManagerFor = (ctx: ExtensionContext) => runtimeManagers.get(ctx.sessionManager.getSessionId());
  const handoffFor = (ctx: ExtensionContext, shortcut: string) => {
    ...
    if (handedOff) ctx.ui.notify(`Subagent work continues in the background (${shortcut}).`, "info");
    return handedOff;
  };
  const createRuntimeManager = dependencies.runtimeManagerFactory ?? ((config: CompatibleSubagentsConfig) => new ForegroundTaskManager({ runner: createSdkForegroundRunner(), catalog: internalAgentCatalog, routePort: manager, config }));
  pi.registerMessageRenderer?.("pi-agent-profiles:subagent-complete", renderSubagentCompletionMessage as never);
  const refreshWidget = (ctx: ExtensionContext) => {
    const sessionId = ctx.sessionManager.getSessionId();
    const runtime = runtimeManagers.get(sessionId);
    let state = backgroundWidgets.get(sessionId);
    if (!state && runtime && typeof (runtime as any).list === "function") { state = new BackgroundWidgetState(() => (runtimeManagers.get(sessionId) as any)?.list?.(sessionId) ?? [], () => refreshWidget(ctx)); backgroundWidgets.set(sessionId, state); }
    (ctx.ui as any).setWidget?.(widgetKey(sessionId), state ? (_tui: any, theme: any) => new BackgroundWidget(state!, theme) : undefined, { placement: "belowEditor" });
  };
  let runtimeHistory: RuntimeHistory | undefined;
  const runtimeContextFor = (ctx: ExtensionContext) => ({ cwd: ctx.cwd, projectTrusted: ctx.isProjectTrusted(), sessionId: ctx.sessionManager.getSessionId(), orchestrator: { model: ctx.model ? { provider: ctx.model.provider, id: ctx.model.id } : undefined, effort: pi.getThinkingLevel() }, ctx, agentDir: getAgentDir(), fs: runtimeFs });
  for (const tool of createForegroundTools(runtimeManagerFor, runtimeContextFor, toolNamespace)) pi.registerTool?.(tool as never);
  ...
  async function load(ctx: ExtensionContext) {
    ...
    const runtimeConfig = readCompatibleSubagentsConfig({ fs: runtimeFs, agentDir: getAgentDir(), cwd: ctx.cwd, projectTrusted: ctx.isProjectTrusted() });
    const sessionId = ctx.sessionManager.getSessionId();
    effectiveHandoffShortcuts.set(sessionId, runtimeConfig.backgroundHandoffShortcut ?? "ctrl+h");
    const existingRuntimeManager = runtimeManagers.get(sessionId);
    if (!runtimeHistory && dependencies.runtimeHistoryFactory) try { runtimeHistory = dependencies.runtimeHistoryFactory(runtimeConfig); } catch { /* runtime remains usable when storage is unavailable */ }
if (existingRuntimeManager) existingRuntimeManager.updateConfig(runtimeConfig);
      else if (dependencies.runtimeManagerFactory) runtimeManagers.set(sessionId, createRuntimeManager(runtimeConfig));
      else {
        if (!runtimeHistory) try { runtimeHistory = new RuntimeHistory(join(getAgentDir(), "pi-agent-profiles", "runtime", "history.sqlite"), runtimeConfig.historyLimit); } catch { /* runtime remains usable when storage is unavailable */ }
        runtimeManagers.set(sessionId, new ForegroundTaskManager({ runner: createSdkForegroundRunner(), catalog: internalAgentCatalog, routePort: manager, config: runtimeConfig, history: runtimeHistory }));
      }
      const runtime = runtimeManagers.get(sessionId);
      runtime?.bindCompletionNotifier?.((task) => pi.sendMessage?.({ customType: "pi-agent-profiles:subagent-complete", content: completionMessage(task), details: completionMessageDetails(task), display: true }, { triggerTurn: true, deliverAs: "followUp" }));
          ...
          if (typeof onTerminalInput === "function") {
            const dispose = onTerminalInput.call(ctx.ui, (data: string) => {
              ...
              const widgetInput = backgroundWidgets.get(sessionId)?.handleTerminalInput(data);
              if (widgetInput?.action?.type === "open-task") void showHistory(ctx, widgetInput.action.taskId);
              if (widgetInput?.consume) return { consume: true };
              if (data === "\u001b") runtimeManagerFor(ctx)?.cancelOnDoubleEscape(sessionId);
              const effectiveShortcut = effectiveHandoffShortcuts.get(sessionId);
              if (effectiveShortcut && effectiveShortcut !== backgroundHandoffShortcut && terminalInputMatchesShortcut(data, effectiveShortcut)) {
                handoffFor(ctx, effectiveShortcut);
                return { consume: true };
              }
              return undefined;
            });
            if (typeof dispose === "function") terminalInputDisposers.set(sessionId, dispose);
          }
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      reacquireRuntime();
      await load(ctx);
      if (namespaceDiagnostic && !reportedNamespaceDiagnostic) { ctx.ui.notify(namespaceDiagnostic, "warning"); reportedNamespaceDiagnostic = true; }
    ...
    const showHistory = async (ctx: ExtensionContext, initialTaskId?: string) => {
        try {
          await load(ctx);
          const sessionId = ctx.sessionManager.getSessionId(); const runtime = runtimeManagerFor(ctx) as any;
          if (!runtime?.list || !runtime?.thread) return void ctx.ui.notify("Subagent history is unavailable for this session.", "error");
              ...
              try {
                ...
                await openHistoryOverlay(ctx, {
                  ...
                  contextWindowForTask: (task) => {
                    const model = task.model?.provider && task.model?.id
          ...
          finally { historyPanels.delete(sessionId); refreshWidget(ctx); }
        } catch (error) {
    ...
    pi.registerCommand("subagents", {
      ...
      handler(_args: string, ctx: ExtensionContext) { return showHistory(ctx); },
    });
    registerCommands(pi, manager, save, load, () =>
    Object.fromEntries(
      ...
      (ctx) => {
        try {
          ...
          const message = "Internal agent catalog is unavailable; using the optional catalog fallback";
          if (!reportedDiscoveryDiagnostics.has(message)) {
            reportedDiscoveryDiagnostics.add(message);
            ctx.ui.notify(message, "warning");
          }
    ...
    pi.registerShortcut(historyPanelShortcut as Parameters<PiLike["registerShortcut"]>[0], {
      description: "Browse subagent history",
      handler(ctx: ExtensionContext) { void showHistory(ctx); },
    });
    pi.registerShortcut(backgroundHandoffShortcut as Parameters<PiLike["registerShortcut"]>[0], {
  ...
  if (!supportedShortcut(shortcut)) {
    shortcutDiagnostic =
      `pi-agent-profiles global shortcut is invalid; using ${DEFAULT_SHORTCUT}`;
  }
  try {
    pi.registerShortcut(
    ...
    shortcutDiagnostic =
      `pi-agent-profiles global shortcut could not be registered; using ${DEFAULT_SHORTCUT}`;
    pi.registerShortcut(
